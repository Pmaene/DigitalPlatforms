...
; This file was generated Thu Dec 13 10:40:32 2012
;--------------------------------------------------------
	.module encryption
...
;	encryption.c:9: volatile __xdata __at (0x4000) unsigned char message[SIZE+1] = {};
	...
	mov	a,#0xB9
	movx	@dptr,a
;	encryption.c:10: volatile __xdata __at (0x4181) unsigned char enc_exp[SIZE] = {0x01,0x01};
	mov	dptr,#_enc_exp
	mov	a,#0x01
	movx	@dptr,a
...
;------------------------------------------------------------
;------------------------------------------------------------
;	encryption.c:16: void write_data() {
;	-----------------------------------------
;	 function write_data
;	-----------------------------------------
_write_data:
	...
	ar0 = 0x00
	ar1 = 0x01
;	encryption.c:17: P1 = 0;
	mov	_P1,#0x00
;	encryption.c:19: P0 = ins_write_data;
	mov	_P0,#0x01
;	encryption.c:20: P0 = ins_idle;
	mov	_P0,#0x00
;	encryption.c:22: while (P1 == 0) {}
00101$:
	mov	a,_P1
	jz	00101$
;	encryption.c:23: P0 = ins_ack;
	mov	_P0,#0x04
	ret
;------------------------------------------------------------
...
;------------------------------------------------------------
;------------------------------------------------------------
;	encryption.c:26: void read_r() {
;	-----------------------------------------
;	 function read_r
;	-----------------------------------------
_read_r:
;	encryption.c:27: P0 = ins_read_r;
	mov	_P0,#0x03
;	encryption.c:28: while (P1 == 0) {}
00101$:
	mov	a,_P1
	jz	00101$
;	encryption.c:29: P0 = ins_ack;
	mov	_P0,#0x04
;	encryption.c:31: P0 = ins_idle;
	mov	_P0,#0x00
	ret
;------------------------------------------------------------
...
;------------------------------------------------------------
;------------------------------------------------------------
;	encryption.c:34: void montgomery_exp() {
;	-----------------------------------------
;	 function montgomery_exp
;	-----------------------------------------
_montgomery_exp:
;	encryption.c:35: P0 = ins_montgomery_exp;
	mov	_P0,#0x02
;	encryption.c:36: P0 = ins_idle;
	mov	_P0,#0x00
	ret
;------------------------------------------------------------
...
;------------------------------------------------------------
;------------------------------------------------------------
;	encryption.c:39: void terminate() {
;	-----------------------------------------
;	 function terminate
;	-----------------------------------------
_terminate:
;	encryption.c:40: P3 = 0x55;
	mov	_P3,#0x55
	ret
;------------------------------------------------------------
...
;------------------------------------------------------------
;------------------------------------------------------------
;	encryption.c:43: int main() {
;	-----------------------------------------
;	 function main
;	-----------------------------------------
_main:
;	encryption.c:44: write_data();
	lcall	_write_data
;	encryption.c:45: montgomery_exp();
	lcall	_montgomery_exp
;	encryption.c:46: read_r();
	lcall	_read_r
;	encryption.c:48: terminate();
	lcall	_terminate
;	encryption.c:49: return 0;
	mov	dptr,#0x0000
	ret
	.area CSEG    (CODE)
	.area CONST   (CODE)
