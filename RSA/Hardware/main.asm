...
; This file was generated Fri Dec  7 10:10:26 2012
;--------------------------------------------------------
	.module main
...
;	main.c:10: volatile __xdata __at (0x0180) unsigned char message[SIZE] = {};
	...
	mov	a,#0xB9
	movx	@dptr,a
;	main.c:11: volatile __xdata __at (0x0200) unsigned char enc_exp[SIZE] = {0x1,0x01};
	mov	dptr,#_enc_exp
	mov	a,#0x01
	movx	@dptr,a
...
;------------------------------------------------------------
;	main.c:24: void write_data(unsigned char *x, unsigned char *r2modm, unsigned char *a, unsigned char *e, unsigned char *m) {
;	-----------------------------------------
;	 function write_data
;	-----------------------------------------
_write_data:
...
00104$:
	...
	subb	a,#0x00
	jnc	00107$
;	main.c:28: shared_x[i] = x[SIZE-1-i];
	mov	r5,_i
	mov	a,#(_shared_x >> 8)
	add	a,(_i + 1)
	mov	r6,a
...
00108$:
	...
	subb	a,#0x00
	jnc	00111$
;	main.c:32: shared_r2modm[i] = r2modm[SIZE-1-i];
	mov	a,_i
...
00112$:
	...
	subb	a,#0x00
	jnc	00115$
;	main.c:36: shared_a[i] = a[SIZE-1-i];
	mov	r2,_i
	mov	a,#(_shared_a >> 8)
	add	a,(_i + 1)
	mov	r3,a
...
00116$:
	...
	subb	a,#0x00
	jnc	00119$
;	main.c:40: shared_e[i] = e[SIZE-1-i];
	mov	a,_i
	add	a,#_shared_e
	mov	r2,a
	mov	a,(_i + 1)
...
00120$:
	...
	subb	a,#0x00
	jnc	00123$
;	main.c:44: shared_m[i] = m[SIZE-1-i];
	mov	r2,_i
	mov	a,#(_shared_m >> 8)
	add	a,(_i + 1)
	mov	r3,a
...
;------------------------------------------------------------
;------------------------------------------------------------
;	main.c:54: void read_r() {
;	-----------------------------------------
;	 function read_r
;	-----------------------------------------
_read_r:
;	main.c:55: P0 = ins_read_r;
	mov	_P0,#0x03
;	main.c:56: while (P1 == 0) {}
00101$:
	mov	a,_P1
	jz	00101$
;	main.c:57: P0 = ins_ack;
	mov	_P0,#0x04
;	main.c:59: P0 = ins_idle;
	mov	_P0,#0x00
	ret
;------------------------------------------------------------
...
;------------------------------------------------------------
;------------------------------------------------------------
;	main.c:62: void montgomery_exp() {
;	-----------------------------------------
;	 function montgomery_exp
;	-----------------------------------------
_montgomery_exp:
;	main.c:63: P0 = ins_montgomery_exp;
	mov	_P0,#0x02
;	main.c:64: P0 = ins_idle;
	mov	_P0,#0x00
	ret
;------------------------------------------------------------
...
;	-----------------------------------------
_main:
;	main.c:72: write_data(message, r2modm, rmodm, enc_exp, modulus);
...
;	main.c:78: for (i = 0; i < SIZE; i++) {
	...
	sjmp	00104$
00107$:
;	main.c:82: P0 = ins_write_data;
	mov	_P0,#0x01
;	main.c:83: P0 = ins_idle;
	mov	_P0,#0x00
;	main.c:85: while (P1 == 0) {}
00101$:
	mov	a,_P1
	jz	00101$
;	main.c:86: P0 = ins_ack;
	mov	_P0,#0x04
;	main.c:88: terminate();
	lcall	_terminate
;	main.c:89: return 0;
