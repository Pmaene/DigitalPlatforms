...
; This file was generated Wed Dec 12 14:41:39 2012
;--------------------------------------------------------
	.module main
...
;	main.c:9: volatile __xdata __at (0x4000) unsigned char message[SIZE+1] = {};
	...
	mov	a,#0xB9
	movx	@dptr,a
;	main.c:10: volatile __xdata __at (0x4181) unsigned char enc_exp[SIZE] = {0x01,0x01};
	mov	dptr,#_enc_exp
	mov	a,#0x01
	movx	@dptr,a
...
;	main.c:18: P1 = 0;
	mov	_P1,#0x00
;	main.c:20: P0 = ins_write_data;
	mov	_P0,#0x01
;	main.c:21: P0 = ins_idle;
	mov	_P0,#0x00
;	main.c:23: while (P1 == 0) {}
00101$:
	mov	a,_P1
	jz	00101$
;	main.c:24: P0 = ins_ack;
	mov	_P0,#0x04
	ret
;------------------------------------------------------------
...
;------------------------------------------------------------
;------------------------------------------------------------
;	main.c:27: void read_r() {
;	-----------------------------------------
;	 function read_r
;	-----------------------------------------
_read_r:
;	main.c:28: P0 = ins_read_r;
	mov	_P0,#0x03
;	main.c:29: while (P1 == 0) {}
00101$:
	mov	a,_P1
	jz	00101$
;	main.c:30: P0 = ins_ack;
	mov	_P0,#0x04
;	main.c:32: P0 = ins_idle;
	mov	_P0,#0x00
	ret
;------------------------------------------------------------
...
;------------------------------------------------------------
;------------------------------------------------------------
;	main.c:35: void montgomery_exp() {
;	-----------------------------------------
;	 function montgomery_exp
;	-----------------------------------------
_montgomery_exp:
;	main.c:36: P0 = ins_montgomery_exp;
	mov	_P0,#0x02
...
;	-----------------------------------------
_main:
;	main.c:45: write_data();
	lcall	_write_data
;	main.c:46: montgomery_exp();
	lcall	_montgomery_exp
;	main.c:47: read_r();
	lcall	_read_r
;	main.c:49: for (i = 0; i < SIZE; i++) {
	clr	a
	mov	_i,a
	mov	(_i + 1),a
00101$:
...
;	main.c:49: for (i = 0; i < SIZE; i++) {
	...
	sjmp	00101$
00104$:
;	main.c:53: message[SIZE] = r[-1];
	mov	dptr,#(_r - 0x0001)
	movx	a,@dptr
	mov	dptr,#(_message + 0x0080)
	movx	@dptr,a
;	main.c:59: write_data();
	lcall	_write_data
;	main.c:60: montgomery_exp();
	lcall	_montgomery_exp
;	main.c:61: read_r();	
	lcall	_read_r
;	main.c:63: terminate();
