...
; This file was generated Mon Jul  1 13:37:04 2013
;--------------------------------------------------------
	.module main
...
;	main.c:8: volatile __xdata __at (0x4300) unsigned char message[SIZE] = {};
	...
	mov	a,#0x98
	movx	@dptr,a
;	main.c:9: volatile __xdata __at (0x4400) unsigned char publicKey[SIZE] = {0x01, 0x01};
	mov	dptr,#_publicKey
	mov	a,#0x01
	movx	@dptr,a
...
_main:
	...
	ar0 = 0x00
	ar1 = 0x01
;	main.c:24: startBrk = 1;
	mov	dptr,#_startBrk
	mov	a,#0x01
	movx	@dptr,a
;	main.c:25: montModExp(encryptedMessage, message, publicKey, modulus, mPrime);
...
;------------------------------------------------------------
;	main.c:31: void montMultiply(unsigned char *m, unsigned char *a, unsigned char *b, unsigned char *n, unsigned char nPrime) {
;	-----------------------------------------
...
;	main.c:39: unsigned char b0 = b[0];
...
00103$:
	...
	ljmp	00106$
00149$:
;	main.c:44: tmp = (unsigned char)(tmp) + a[j]*b[i-j];
	push	ar4
...
;	main.c:46: t = t + (tmp >> 8);
	...
	mov	(_montMultiply_t_1_1 + 1),a
;	main.c:48: tmp = (unsigned char)(tmp) + m[j]*n[i-j];
...
;	main.c:43: for (j = 0; j < i; j++) {
	...
	clr	a
	cjne	a,_montMultiply_j_1_1,00150$
	inc	(_montMultiply_j_1_1 + 1)
00150$:
	pop	ar4
	ljmp	00103$
00106$:
;	main.c:52: tmp = (unsigned char)(tmp) + a[i]*b0;
	push	ar4
	mov	r7,_montMultiply_tmp_1_1
...
;	main.c:53: t = t + (tmp >> 8);
	...
	addc	a,(_montMultiply_t_1_1 + 1)
	mov	(_montMultiply_t_1_1 + 1),a
;	main.c:55: c = (unsigned char)(tmp)*nPrime;
...
;	main.c:56: m[i] = c;
	...
	lcall	__gptrput
;	main.c:58: tmp = (unsigned char)(tmp) + c*n0;
...
;	main.c:42: for (i = 0; i < SIZE; i++) {
	...
	clr	a
	cjne	a,_montMultiply_i_1_1,00151$
	inc	(_montMultiply_i_1_1 + 1)
00151$:
	pop	ar4
	ljmp	00107$
00110$:
;	main.c:63: for (i = SIZE; i < 2*SIZE; i++) {
...
00115$:
	...
	ljmp	00118$
00152$:
;	main.c:64: for (j = i-SIZE+1; j < SIZE; j++) {
	mov	a,#0x81
...
00111$:
	...
	ljmp	00114$
00153$:
;	main.c:65: tmp = (unsigned char)(tmp)  + a[j]*b[i-j];
	push	ar4
...
;	main.c:66: t = t + (tmp >> 8);
	...
	addc	a,(_montMultiply_t_1_1 + 1)
	mov	(_montMultiply_t_1_1 + 1),a
;	main.c:68: tmp = (unsigned char)(tmp) + m[j]*n[i-j];
	mov	r5,_montMultiply_tmp_1_1
	mov	_montMultiply_sloc3_1_0,r5
	mov	(_montMultiply_sloc3_1_0 + 1),#0x00
	mov	a,r7
...
00154$:
	...
	ljmp	00111$
00114$:
;	main.c:72: m[i-SIZE] = (unsigned char)(tmp);
	mov	a,_montMultiply_i_1_1
	add	a,#0x80
	mov	r5,a
	mov	a,(_montMultiply_i_1_1 + 1)
...
;	main.c:63: for (i = SIZE; i < 2*SIZE; i++) {
	inc	_montMultiply_i_1_1
	clr	a
	cjne	a,_montMultiply_i_1_1,00155$
	inc	(_montMultiply_i_1_1 + 1)
00155$:
	ljmp	00115$
00118$:
;	main.c:77: m[SIZE] = tmp;
	mov	a,#0x80
	add	a,_montMultiply_m_1_1
	mov	r5,a
	clr	a
...
;	main.c:80: for (i=0; i < SIZE; i++) {
	...
	sjmp	00119$
00122$:
;	main.c:85: if ((tmp >> 8) == 0) {
	mov	r2,(_montMultiply_tmp_1_1 + 1)
	mov	r3,#0x00
	mov	a,r2
	orl	a,r3
;	main.c:86: for (i = 0; i < SIZE; i++)
	jnz	00127$
	mov	r2,a
	mov	r3,a
00123$:
...
;------------------------------------------------------------
;	main.c:91: void montModExp(unsigned char *m, unsigned char *x, unsigned char *e, unsigned char *n, unsigned char nPrime) {
;	-----------------------------------------
;	 function montModExp
;	-----------------------------------------
_montModExp:
...
;	main.c:102: montMultiply(xTilde, x, r2modm, n, nPrime);
...
00103$:
	...
	subb	a,#0x80
	mov	a,(_montModExp_i_1_1 + 1)
	subb	a,#0x00
	jnc	00106$
;	main.c:104: m[i] = rmodm[i];
	mov	a,_montModExp_i_1_1
	add	a,r2
	mov	r1,a
	mov	a,(_montModExp_i_1_1 + 1)
	...
	mov	r5,a
	mov	ar6,r4
	mov	dpl,_montModExp_i_1_1
	mov	a,#(_rmodm >> 8)
	add	a,(_montModExp_i_1_1 + 1)
	mov	dph,a
	movx	a,@dptr
	mov	r7,a
	mov	dpl,r1
...
;	main.c:103: for (i = 0; i < SIZE; i++)
	...
	clr	a
	cjne	a,_montModExp_i_1_1,00103$
	inc	(_montModExp_i_1_1 + 1)
	sjmp	00103$
00106$:
;	main.c:106: for (i = 0; i <= t; i++) {
	clr	a
	mov	_montModExp_i_1_1,a
	mov	(_montModExp_i_1_1 + 1),a
00107$:
	...
	ljmp	00110$
00121$:
;	main.c:107: montMultiply(m, m, m, n, nPrime);
...
00123$:
	djnz	b,00122$
	jnb	acc.0,00109$
;	main.c:109: montMultiply(m, m, xTilde, n, nPrime);
...
;	main.c:106: for (i = 0; i <= t; i++) {
	inc	_montModExp_i_1_1
	clr	a
	cjne	a,_montModExp_i_1_1,00125$
	inc	(_montModExp_i_1_1 + 1)
00125$:
	ljmp	00107$
00110$:
;	main.c:112: montMultiply(m, m, one, n, nPrime);
...
;------------------------------------------------------------
;	main.c:115: unsigned short _findFirstOne(unsigned char *e) {
;	-----------------------------------------
...
;	main.c:120: if (e[(SIZE-1)-i] != 0) {
	...
	ljmp	00111$
00122$:
;	main.c:121: for (j = 0; j < 8; j++) {
	mov	r6,#0x00
00105$:
	cjne	r6,#0x08,00123$
00123$:
	jnc	00111$
;	main.c:122: if (e[(SIZE-1)-i] >> (7-j) & 1)
...
00126$:
	djnz	b,00125$
	jnb	acc.0,00107$
;	main.c:123: return 8*((SIZE-1)-i) + (7-j);
	mov	a,#0x7F
	clr	c
	subb	a,r7
	mov	r7,a
