#include <8051.h>
#include <string.h>

#define SIZE 128

volatile __xdata __at (0x4000) unsigned char rmodm[SIZE] = {};
volatile __xdata __at (0x4100) unsigned char r2modm[SIZE] = {};

volatile __xdata __at (0x4200) unsigned char modulus[SIZE] = {};
volatile __xdata __at (0x4300) unsigned char message[SIZE] = {};
volatile __xdata __at (0x4400) unsigned char publicKey[SIZE] = {0x01, 0x01};
volatile __xdata __at (0x4500) unsigned char privateKey[SIZE] = {};

volatile __xdata __at (0x5000) unsigned char encryptedMessage[SIZE];

volatile __xdata __at (0xFFFD) unsigned char startBrk;
volatile __xdata __at (0xFFFE) unsigned char innerBrk;
volatile __xdata __at (0xFFFF) unsigned char endBrk;

void montMultiply(unsigned char *m, unsigned char *a, unsigned char *b, unsigned char *n, unsigned char nPrime);
void montModExp(unsigned char *m, unsigned char *x, unsigned char *e, unsigned char *n, unsigned char nPrime);
unsigned short _findFirstOne(unsigned char *e);

int main() {
    unsigned char mPrime = 0xBD;
    __xdata __at (0x2000) unsigned char t2a[SIZE] = {};
    __xdata __at (0x2100) unsigned char t2b[SIZE] = {};
    __xdata __at (0x2200) unsigned char t3[SIZE];

    startBrk = 1;
    //montModExp(encryptedMessage, message, publicKey, modulus, mPrime);
    montMultiply(t3, t2a, t2b, modulus, mPrime);
    endBrk = 1;

    return 0;
}

void montMultiply(unsigned char *m, unsigned char *a, unsigned char *b, unsigned char *n, unsigned char nPrime) {
    unsigned short tmp = 0;
    unsigned short i = 0;
    unsigned short j = 0;
    unsigned short t = 0;

    unsigned char c = 0;
    unsigned char n0 = n[0];
    unsigned char b0 = b[0];

    __xdata __at (0x1000) unsigned char u[SIZE + 1];

    for (i = 0; i < SIZE; i++) {
        for (j = 0; j < i; j++) {
            tmp = (unsigned char)(tmp) + a[j]*b[i-j];

            t = t + (tmp >> 8);

            tmp = (unsigned char)(tmp) + m[j]*n[i-j];
            t = t + (tmp >> 8);
        }

        tmp = (unsigned char)(tmp) + a[i]*b0;
        t = t + (tmp >> 8);

        c = (unsigned char)(tmp)*nPrime;
        m[i] = c;

        tmp = (unsigned char)(tmp) + c*n0;
        tmp = t + (tmp >> 8);
        t = tmp >> 8;
    }

    for (i = SIZE; i < 2*SIZE; i++) {
        for (j = i-SIZE+1; j < SIZE; j++) {
            tmp = (unsigned char)(tmp)  + a[j]*b[i-j];
            t = t + (tmp >> 8);

            tmp = (unsigned char)(tmp) + m[j]*n[i-j];
            t = t + (tmp >> 8);
        }

        m[i-SIZE] = (unsigned char)(tmp);
        tmp = (unsigned char)(t);
        t = t >> 8;
    }

    m[SIZE] = tmp;
    tmp = m[0] - n[0];

    for (i=0; i < SIZE; i++) {
        tmp = m[i] - n[i] - (tmp >> 8);
        u[i] = tmp;
    }

    if ((tmp >> 8) == 0)
        memcpy(m, u, SIZE);
}

void montModExp(unsigned char *m, unsigned char *x, unsigned char *e, unsigned char *n, unsigned char nPrime) {
    __xdata __at (0x1100) unsigned char one[SIZE];
    __xdata __at (0x1200) unsigned char xTilde[SIZE];
    __xdata __at (0x1300) unsigned char a[SIZE];

    unsigned short i = 0;
    unsigned short t = 0;

    one[SIZE-1] = 0x01;

    t = _findFirstOne(e);

    montMultiply(xTilde, x, r2modm, n, nPrime);
    memcpy(m, rmodm, SIZE);

    for (i = 0; i <= t; i++) {
        montMultiply(a, m, m, n, nPrime);
        if ((e[(t-i)/8] >> (t-i)%8) & 1) {
            montMultiply(m, a, xTilde, n, nPrime);
        } else {
            memcpy(m, a, SIZE);
        }

        innerBrk = 1;
    }

    montMultiply(a, m, one, n, nPrime);
    memcpy(m, a, SIZE);
}

unsigned short _findFirstOne(unsigned char *e) {
    unsigned char i = 0;
    unsigned char j = 0;

    for (i = 0; i < SIZE; i++) {
        if (e[(SIZE-1)-i] != 0) {
            for (j = 0; j < 8; j++) {
                if (e[(SIZE-1)-i] >> (7-j) & 1)
                    return 8*((SIZE-1)-i) + (7-j);
            }
        }
    }

    return 0;
}
