...
; This file was generated Mon Nov  5 10:06:08 2012
;--------------------------------------------------------
	.module main
...
;	main.c:16: __xdata __at (0x0100) unsigned char b[SIZE] = {};
	...
	mov	a,#0xBE
	movx	@dptr,a
;	main.c:18: __xdata __at (0x0180) unsigned char n_prime = 0xA5;
	mov	dptr,#_main_n_prime_1_1
	mov	a,#0xA5
	movx	@dptr,a
;	main.c:22: startBrk=1;
	mov	dptr,#_startBrk
	mov	a,#0x01
	movx	@dptr,a
;	main.c:24: fips(a, b, n, n_prime, m);
...
;------------------------------------------------------------
;	main.c:30: void fips(unsigned char *a, unsigned char *b, unsigned char *n, unsigned char n_prime, unsigned char *m) {
;	-----------------------------------------
;	 function fips
;	-----------------------------------------
_fips:
	...
	mov	(_fips_a_1_1 + 2),b
;	main.c:38: signed   char  u    = 0;
	mov	r5,#0x00
;	main.c:41: unsigned char  t[3] = {0, 0, 0};
	mov	_fips_t_1_1,#0x00
	mov	(_fips_t_1_1 + 0x0001),#0x00
	mov	(_fips_t_1_1 + 0x0002),#0x00
;	main.c:45: for (i = 0; i < SIZE; i++) {
...
;	main.c:71: t[1] = tmp;
	mov	r2,_fips_tmp_1_1
	mov	(_fips_t_1_1 + 0x0001),r2
;	main.c:72: t[2] = t[2] + (tmp >> 8);
	mov	a,(_fips_tmp_1_1 + 1)
	add	a,(_fips_t_1_1 + 0x0002)
	mov	r3,a
	mov	(_fips_t_1_1 + 0x0002),r3
;	main.c:74: m[i] = (s*n_prime)%256;
	mov	a,_fips_i_1_1
	add	a,_fips_PARM_5
...
00148$:
	...
	ljmp	00107$
00110$:
;	main.c:89: for (i = SIZE; i < 2*SIZE; i++) {
...
00115$:
	...
	ljmp	00118$
00149$:
;	main.c:90: for (j = i-SIZE+1; j < SIZE; j++) {
	mov	a,#0x81
	add	a,_fips_i_1_1
	mov	r4,a
	mov	a,#0xFF
...
;	main.c:96: t[1] = tmp;
	mov	r2,_fips_tmp_1_1
	mov	(_fips_t_1_1 + 0x0001),r2
;	main.c:97: t[2] = t[2] + (tmp >> 8);
	mov	a,(_fips_tmp_1_1 + 1)
	add	a,(_fips_t_1_1 + 0x0002)
	mov	_fips_sloc3_1_0,a
	mov	(_fips_t_1_1 + 0x0002),_fips_sloc3_1_0
;	main.c:99: tmp = s + m[j]*n[i-j];
...
;------------------------------------------------------------
;c                         Allocated with name '_add_PARM_2'
;t                         Allocated with name '_add_t_1_1'
;tmp                       Allocated to registers r0 r1 
;------------------------------------------------------------
;	main.c:138: void add(unsigned char *t, unsigned char c) {
;	-----------------------------------------
;	 function add
;	-----------------------------------------
_add:
	mov	_add_t_1_1,dpl
	mov	(_add_t_1_1 + 1),dph
	mov	(_add_t_1_1 + 2),b
;	main.c:139: unsigned short tmp = t[1] + c;
	mov	a,#0x01
	add	a,_add_t_1_1
	mov	r5,a
	clr	a
